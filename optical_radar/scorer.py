...
def score_paper(
    ...
    for group in SUPPORTING_GROUPS:
        keyword_score += _group_score(match_locations, group, title_weight=12, abstract_weight=10, metadata_weight=3, max_score=25)
    ...
    if title_matches:
        keyword_score += min(len(set(keyword.lower() for keyword in title_matches)) * 8, 18)
    ...
    score = keyword_score + source_quality_score - penalty_score

    if has_supporting_hit and not has_topic_hit:
        score = min(score, 20)
    if not has_positive_keyword_hit:
        score = min(source_quality_score or score, 15)
