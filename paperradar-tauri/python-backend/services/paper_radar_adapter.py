...
def _source_label(paper: Paper) -> str:
    source_type = (paper.source_type or "").lower()
    source = (paper.journal_or_source or "").strip()
    if source_type == "arxiv":
        return "arXiv"
    if "nature communications" in source.lower():
        return "Nature Comm."
    if source:
        return source[:40]
    if source_type in {"crossref", "journal_rss"}:
        return "顶级期刊"
    return "Other"
