...
function sourceVariant(source: Paper["source"]) {
  if (source === "arXiv") return "blue" as const;
  if (source === "Nature" || source === "Nature Comm." || source === "顶级期刊") return "purple" as const;
      ...
      {papers.length === 0 ? (
        ...
        <div className="max-h-[620px] min-h-[420px] overflow-auto">
          <div className="min-w-[1280px]">
            <div className="paper-table-grid sticky top-0 z-10 grid border-b border-slate-200 bg-slate-50/95 px-4 py-3 text-xs font-semibold text-slate-500 backdrop-blur">
              <div>分数</div>
              <div>来源</div>
              <div>标题</div>
              <div>作者</div>
            ...
            {papers.map((paper) => (
              ...
              >
                ...
                <div>
                  <Badge className="min-w-[86px]" variant={sourceVariant(paper.source)}>
                    {paper.source}
                  </Badge>
                </div>
                <div
                  className="truncate pr-5 font-medium text-slate-900 hover:text-blue-700"
                  title={`${paper.title}\n双击打开论文链接`}
                >
                  {paper.title}
                </div>
                <div className="truncate pr-4 text-slate-600" title={paper.authors.join(", ")}>
                  {shortAuthors(paper.authors)}
                </div>
                <div className="text-slate-500">{paper.publishedDate}</div>
                <div className="flex min-w-0 gap-1.5 overflow-hidden pr-3">
                  {paper.matchedKeywords.slice(0, 3).map((keyword) => (
                    <Badge key={keyword} variant="slate" className="min-w-0 max-w-[92px] truncate px-2 font-medium">
                      {keyword}
                    </Badge>
                  ))}
                </div>
                <div className="flex justify-center">
                  <Button
                    ...
                    className={cn("h-8 w-[72px]", paper.url && "text-blue-700")}
                  >
                    {paper.url ? (
                      <>
